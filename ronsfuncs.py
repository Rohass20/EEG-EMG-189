...
def plot_testing_accuracy_reg(participant, type, dframe):
    ...
    if 'loo' in participant:
        ...
        meeg, beeg = np.polyfit(x=x, y=df[df.index.str.contains('eeg')].to_numpy().flatten(), deg=2)
        memg, bemg = np.polyfit(x=x, y=df[df.index.str.contains('emg')].to_numpy().flatten(), deg=2)
        mboth, bboth = np.polyfit(x=x, y=df[df.index.str.contains('both')].to_numpy().flatten(), deg=2)
        ...
        meeg, beeg = np.polyfit(x=x, y=df[df.index.str.contains('eeg')].to_numpy().flatten()[:25], deg=2)
        memg, bemg = np.polyfit(x=x, y=df[df.index.str.contains('emg')].to_numpy().flatten()[:25], deg=2)
        mboth, bboth = np.polyfit(x=x, y=df[df.index.str.contains('both')].to_numpy().flatten()[:25], deg=2)
